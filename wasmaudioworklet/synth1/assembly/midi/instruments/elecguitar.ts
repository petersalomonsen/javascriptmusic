// Faust-generated Electric Guitar Physical Model
// Transpiled from Faust DSP: elecGuitarMIDI.dsp
// Original: (c) Romain Michon, CCRMA (Stanford University), GRAME, MIT license
//
// Faust source:
//   import("stdfaust.lib");
...
function faustpower2_f(value: f32): f32 {
    return value * value;
}

export class ElecGuitar extends MidiVoice {
    // --- Faust UI parameters ---
    private fHslider0: f32 = 0.5;       // outGain (0..1)
    private fHslider1: f32 = 0.8;       // pluckPosition (0..1)
    private fHslider2: f32 = 440.0;     // freq (50..1000)
    private fButton0: f32 = 0.0;        // gate
    private fHslider3: f32 = 0.0;       // sustain (0..1)
    private fHslider4: f32 = 0.0;       // bend (-2..2 semitones)
    private fHslider5: f32 = 0.8;       // gain (0..1)

    // --- Faust sample-rate-dependent constants ---
    private fConst1: f32;
    private fConst2: f32;
    private fConst3: f32;
    private fConst4: f32;
    ...
    // --- Faust DSP state (delay lines / recursion buffers) ---
    ...
    private instanceClear(): void {
        for (let i = 0; i < 2; i++) { this.fRec29[i] = 0; }
        for (let i = 0; i < 2; i++) { this.fVec0[i] = 0; }
        for (let i = 0; i < 2; i++) { this.fRec30[i] = 0; }
        for (let i = 0; i < 2; i++) { this.fRec25[i] = 0; }
        for (let i = 0; i < 2; i++) { this.fRec31[i] = 0; }
        for (let i = 0; i < 4; i++) { this.fRec33[i] = 0; }
        this.IOTA = 0;
        for (let i = 0; i < 2048; i++) { this.fRec34[i] = 0; }
        for (let i = 0; i < 2; i++) { this.fVec1[i] = 0; }
        for (let i = 0; i < 2; i++) { this.iRec36[i] = 0; }
        for (let i = 0; i < 3; i++) { this.fRec35[i] = 0; }
        for (let i = 0; i < 2; i++) { this.iRec37[i] = 0; }
        for (let i = 0; i < 3; i++) { this.fVec2[i] = 0; }
        for (let i = 0; i < 2048; i++) { this.fRec32[i] = 0; }
        for (let i = 0; i < 2; i++) { this.fRec21[i] = 0; }
        for (let i = 0; i < 2; i++) { this.fRec17[i] = 0; }
        for (let i = 0; i < 2048; i++) { this.fRec13[i] = 0; }
        for (let i = 0; i < 2; i++) { this.fRec15[i] = 0; }
        for (let i = 0; i < 4; i++) { this.fRec11[i] = 0; }
        for (let i = 0; i < 2; i++) { this.fRec6[i] = 0; }
        for (let i = 0; i < 2048; i++) { this.fRec2[i] = 0; }
        for (let i = 0; i < 2; i++) { this.fRec0[i] = 0; }
    }

    noteon(note: u8, velocity: u8): void {
        super.noteon(note, velocity);
        this.fHslider2 = notefreq(note as f32);
        this.fButton0 = 1.0;
    ...
    nextframe(): void {
        // --- Faust compute() body for ONE sample ---
        // (direct transpilation from C output lines 423-572)

        const fSlow0: f32 = this.fHslider0;
        const fSlow1: f32 = this.fConst3 * this.fHslider1;
        const fSlow2: f32 = this.fHslider2;
        const fSlow3: f32 = 340.0 / fSlow2;
        const fSlow4: f32 = Mathf.min(1.0, this.fButton0 + this.fHslider3);
        const iSlow5: i32 = fSlow4 == 0.0 ? 1 : 0;
        const fSlow6: f32 = Mathf.pow(2.0, 0.0833333358 * this.fHslider4);
        const fSlow7: f32 = this.fHslider5;
        const fSlow8: f32 = this.fConst5 * fSlow2;
        const fSlow9: f32 = 0.000500000024 * fSlow2;

        // --- Begin single-sample compute ---
        ...
        const iTemp5: i32 = <i32>Mathf.min(this.fConst1, <f32>max(0, iTemp4));
        ...
        const iTemp31: i32 = <i32>Mathf.min(this.fConst1, <f32>max(0, iTemp30));
        ...
        // Output mono signal to channel (level=0.5, pan=0.5 center)
