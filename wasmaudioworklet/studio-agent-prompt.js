...
- **The song source runs as ONE top-level async function — \`await\` works directly at top level.** Write \`await createTrack(0).steps(...)\` as a plain statement. NEVER wrap sequencing in an async IIFE (\`(async () => { ... })()\`) or any wrapper function: the IIFE is not awaited, so the code after it — including \`loopHere()\` — runs at beat 0 before any notes are scheduled and the song breaks. No wrappers are ever needed.

- **Tempo / flow:** \`setBPM(bpm)\`; \`await waitForBeat(beat)\` (absolute) or \`await waitDuration(beats)\` (relative, advances the clock) — both exist as globals AND track methods; \`playFromHere()\`. **PREFER \`waitDuration\` for sequencing sections back-to-back** (\`playIntro(); await waitDuration(16); playGroove(); await waitDuration(16); ...\`): each section advances relative to where the last ended, so inserting, removing, or reordering a section doesn't force you to recompute every following beat. Reserve \`waitForBeat\` for pinning an event to a specific absolute beat. If you find yourself hand-computing cumulative beats (64 → 80 → 96 …), switch to \`waitDuration\`.
