...
\`resolution\`, \`time\` (seconds ≈ song time), \`targetNoteStates[128]\` / \`smoothedNoteStates[128]\` (per MIDI note, **-1 = no note**, sounding ≈ velocity/127*2-1; the smoothed one has instant attack, slow release), \`synthState[]\` (raw f32 the synth writes in postprocess), \`uSampler\`/\`uSamplerPrev\`/\`uMix\` (addImage/startVideo layer), \`uText\`/\`uTextPrev\`/\`uTextMix\` (showText layer, alpha = glyph coverage), and any \`uniform float <name>\` the song sets with setVisual. The context is WebGL2 and shaders are GLSL ES 1.00 by default (no \`switch\`, loops need constant bounds, always \`precision highp float;\`); a first line of \`#version 300 es\` opts into GLSL ES 3.00 (\`out vec4 fragColor\` instead of \`gl_FragColor\`, \`texture()\` instead of \`texture2D\`, dynamic loop bounds, \`texelFetch\`) — keep an existing shader in the dialect it is in. Texture rows upload top-first, so sample with \`vec2(uv.x, 1.0 - uv.y)\`.
